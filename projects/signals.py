...
def create_endtime_of_project(sender, instance, created, **kwargs):
    if created and instance.dura_converter == instance.ONE_DAY:
        Project.objects.filter(pk=instance.id).update(duration = one_day())
    if created and instance.dura_converter == instance.TWO_DAYS:
        Project.objects.filter(pk=instance.id).update(duration = two_days())
    if created and instance.dura_converter == instance.THREE_DAYS:
        Project.objects.filter(pk=instance.id).update(duration = three_days())
    if created and instance.dura_converter == instance.FOUR_DAYS:
        Project.objects.filter(pk=instance.id).update(duration = four_days())
    if created and instance.dura_converter == instance.FIVE_DAYS:
        Project.objects.filter(pk=instance.id).update(duration = five_days())
    if created and instance.dura_converter == instance.SIX_DAYS:
        Project.objects.filter(pk=instance.id).update(duration = six_days())
    if created and instance.dura_converter == instance.ONE_WEEK:
        Project.objects.filter(pk=instance.id).update(duration = one_week())
    if created and instance.dura_converter == instance.TWO_WEEK:
        Project.objects.filter(pk=instance.id).update(duration = two_weeks())
    if created and instance.dura_converter == instance.THREE_WEEK:
        Project.objects.filter(pk=instance.id).update(duration = three_weeks())
    if created and instance.dura_converter == instance.ONE_MONTH:
