...
class ProposalAdmin(admin.ModelAdmin):
    list_display = ['image_tag', 'title', 'team','category', 'created_by', 'salary', 'status', 'published']
    prepopulated_fields = {'slug': ('title',)}
    list_display_links = ['image_tag', 'title', ]
    list_editable = [ 'status','published']
    search_fields = ['team__title', 'title', 'category__name']
    readonly_fields = ['image_tag','reference']
    actions = ['Go_Public_on_home', 'Go_Private_on_home']
    fieldsets = (
        ('Basic Info', {'fields': ('title', 'slug', 'created_by',)}),
        ('Classification', {'fields': ('team', 'category','reference', 'published','file_type',)}),
        ('Description', {'fields': ('description','skill',)}),
        ('FAQs', {'fields': ('faq_one','faq_one_description', 'faq_two', 'faq_two_description', 'faq_three', 'faq_three_description',)}),
        ('Specifications', {'fields': ('salary','service_level', 'revision', 'dura_converter', 'duration',)}),
