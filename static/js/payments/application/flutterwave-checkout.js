...
flutterwaveButton.addEventListener('click', function () {
    ...
    fetch("/application/flutter_payment_intent/", {
        method: 'GET',
        ...
        if (data) {
            FlutterwaveCheckout({
                ...
                callback: function (response) {
                    fetch(`/application/flutter_success/?tx_ref=${response.tx_ref}&transaction_id=${response.transaction_id}&status=${response.status}`,{
                        method: 'GET',
                        credentials: 'same-origin',
                        headers: {
                          'Content-Type': 'application/json',
