# Generated by Django 3.2.8 on 2022-08-29 15:43

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('contract', '0010_auto_20220829_1526'),
    ]

    operations = [
        migrations.RemoveField(
            model_name='contract',
            name='status',
        ),
        migrations.AddField(
            model_name='contract',
            name='reaction',
            field=models.CharField(choices=[('awaiting', 'Awaiting'), ('accepted', 'Accepted'), ('rejected', 'Rejected'), ('paid', 'Paid')], default='awaiting', max_length=30, verbose_name='State'),
        ),
        migrations.AlterField(
            model_name='contract',
            name='contract_duration',
            field=models.CharField(choices=[('one_day', '01 Day'), ('two_days', '02 Days'), ('three_days', '03 Days'), ('four_days', '04 Days'), ('five_days', '05 Days'), ('six_days', '06 Days'), ('one_week', '01 Week'), ('two_weeks', '02 Weeks'), ('three_weeks', '03 Weeks'), ('one_month', '01 Month'), ('two_month', '02 Months'), ('three_months', '03 Months'), ('four_months', '04 Months'), ('five_months', '05 Months'), ('six_months', '06 Months')], default='one_day', max_length=20, verbose_name='Duration'),
        ),
        migrations.AlterField(
            model_name='contract',
            name='reference',
            field=models.CharField(blank=True, max_length=100, null=True, unique=True, verbose_name='Reference'),
        ),
        migrations.AlterField(
            model_name='contract',
            name='slug',
            field=models.SlugField(blank=True, max_length=100, null=True, verbose_name='Slug'),
        ),
        migrations.AlterField(
            model_name='internalcontract',
            name='created_by',
            field=models.ForeignKey(default=3, on_delete=django.db.models.deletion.CASCADE, related_name='internalcontractauthor', to='account.customer', verbose_name='Author'),
            preserve_default=False,
        ),
    ]
